...
- 桌面 OCR：按 `references/practical/桌面OCR对话接入.md` 处理；保留错字、截断和说话人不确定性，不能把左右布局直接当身份确认。
...
可发送话术按实战话术编排器的“自然口吻”规则生成：判断可以细，发出去的话要像用户自己说的。优先跟随已确认的用户原话；没有样本就用朴素口语。不要把分析术语、心理咨询式承接、土味情话或精心设计的段子塞进回复，不靠乱加语气词、表情和错别字伪装自然。边界与真诚保留在行动里，理由和代价放在给用户看的说明里。
...
| 微信窗口 OCR、桌面副驾对话 | `references/practical/桌面OCR对话接入.md` |
