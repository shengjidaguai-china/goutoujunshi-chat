...
**状态：源码已集成，尚未在 Windows 实机验收。** 构建成功时 GitHub Actions 会提供预览包；它不代表已完成实机测试。Mac 版的原文人工核对页、关系 K 线和关系档案管理页尚未移植到这里。OCR 可能认错说话人，填入前必须核对当前会话、收件人和文字；程序只填草稿，不自动发送。
...
首次启动在设置页填写 Jev 判断接口与回复生成接口的 Key；密钥写入当前 Windows 用户的 `GOUTOU_JEV_API_KEY` 与 `GOUTOU_LLM_API_KEY` 环境变量，不会读取原版 Jev 安装时保存的 Key。其他设置写在本目录的 `config.json`，已被 Git 忽略。`build.bat` 可在 Windows 上打包本项目的可执行目录。
